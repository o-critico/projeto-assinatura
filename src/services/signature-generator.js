...
async function signatureGenerator(user, layouts) {
  ...
  layouts.forEach( (layout) => {
    ...
    if (layout.name == 'firstName') {
      context.fillText(`${user.firstName} ${user.lastName}`, layout.x, layout.y);
    } else {
  ...
  const filePath = `/home/mess/Files/Node/projeto-assinatura/public/results/${user.firstName+user.lastName}.png`;
